...
CRequestsProcessorMT::CRequestsProcessorMT(event_base* eventBaseStruct, CSettings& settings) :
	CRequestsProcessor(eventBaseStruct, settings)
{
...
void CRequestsProcessorMT::start(int workersCount)
{
	for (int i = 0; i < workersCount; ++i) {
		TRequestsWorkerUniquePtr worker = std::make_unique<CRequestsWorkerMT>(requests_, requestsQueueMutex_, eventBaseStruct_, settings_);
		if (worker->start()) {
